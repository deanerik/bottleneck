...
verm50 <- lapply(vermMods,function(x){

                    get.plotdata(  x[x$year %in% (2040:2050),])
})

verm100 <- lapply(vermMods,function(x){
...
gen50 <- lapply(genMods,function(x){
                    get.plotdata(  x[x$year %in% (2040:2050),])
})

gen100 <- lapply(genMods,function(x){
                    get.plotdata(  x[x$year %in% (2090:2100),])
})
...
slou50 <- lapply(slouMods,function(x){
                    get.plotdata(  x[x$year %in% (2040:2050),])
})

slou100 <- lapply(slouMods,function(x){
                    get.plotdata(  x[x$year %in% (2090:2100),])
})
...
nip50 <- lapply(nipMods,function(x){
                    get.plotdata(  x[x$year %in% (2040:2050),])
})

nip100 <- lapply(nipMods,function(x){
                    get.plotdata(  x[x$year %in% (2090:2100),])
})
...
surPlot <- ggplot(perSpaSurv, aes(x = year, y = percent, color = type, shape = type)) +
           ...
           #scale_color_manual(values = c(1,2,3,5)) +
           ...
           labs(x = "Year", y = "Cohort survival fraction") +
